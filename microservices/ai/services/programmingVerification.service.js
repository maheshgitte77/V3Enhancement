...
const EXECUTION_TIMEOUT_MS =
  Number(process.env.QUESTION_TEST_EXECUTION_TIMEOUT_MS) || 15000; // Judge0 API timeout (default 15s)
const VERIFICATION_QUESTION_TIMEOUT_MS =
  Number(process.env.VERIFICATION_QUESTION_TIMEOUT_MS) || 0; // 0 = no timeout, wait for full verification (default). Set e.g. 300000 for 5 min cap.
...
const ENABLE_PHASE_38 =
  !SKIP_RECOVERY_PHASES && process.env.ENABLE_VERIFICATION_PHASE_38 !== "false";
// Max concurrent questions verified in parallel (0 = unlimited). Reduces rate limits when many questions.
const QUESTION_CONCURRENCY = Math.max(
  0,
  parseInt(process.env.VERIFICATION_QUESTION_CONCURRENCY, 10) || 0,
);
// Remove failed test cases when 70%+ languages have same partial pass (e.g. 4/5 -> 4/4). Default: true.
const REMOVE_FAILED_TEST_CASES =
  process.env.REMOVE_FAILED_TEST_CASES !== "false";

const QUESTION_SERVICE_URL = process.env.HIRECORRECTO_MAIN_SERVICE_URL;
...
const SINGLE_EXECUTION_URL = joinUrl(
  ...
  return languageList
    .map((lang) => ({
      ...
      codeSnippet: sanitizeText(
        lang?.codeSnippet ||
          lang?.starterCode ||
          boilerplateMap?.[lang?.languageName] ||
          "",
      ),
    }))
    .filter(
      (lang) => Number.isFinite(Number(lang.languageId)) && lang.languageName,
    );
};
...
const getSignaturePattern = (family) => {
  switch (family) {
    case "python":
      return /^\s*def\s+solve\s*\([^)]*\)\s*:/;
    case "kotlin":
      return /fun\s+solve\s*\(/;
    case "go":
      return /func\s+solve\s*\(/;
    case "r":
      return /solve\s*<-\s*function\s*\(/;
    case "haskell":
      return /^solve\s+[\w\s]+\=\s*(do)?\s*$/;
    case "php":
      return /function\s+solve\s*\(/;
    case "dart":
      return /[\w<>]+\s+solve\s*\(/;
    case "rust":
      return /fn\s+solve\s*\(/;
    case "ruby":
      return /def\s+solve\s*\(/;
    case "swift":
      return /func\s+solve\s*\(/;
    case "scala":
      return /def\s+solve\s*\(/;
    case "pascal":
      return /function\s+solve\s*\(|procedure\s+solve\s*\(/;
    case "lua":
      return /function\s+solve\s*\(/;
    case "ocaml":
      return /let\s+solve\s+[^=]+=\s*/;
    case "octave":
      return /function\s+(?:\[[\w\s,]+\]|\w+)\s*=\s*solve\s*\(|function\s+solve\s*\(/;
    case "c":
    case "cpp":
    case "java":
    default:
      return /\bsolve\s*\(|^\s*\w+[\*\s]*\s*solve\s*\(/;
  }
};
...
const normalizePythonStdin = (code = "") => {
  ...
  c = c.replace(
    /\bint\s*\(\s*input\s*\(\s*\)\s*\)/g,
    "int(sys.stdin.readline())",
  );
  c = c.replace(
    /\binput\s*\(\s*\)\s*\.split\s*\(\s*\)/g,
...
const ensureBoilerplateMarkers = (codeSnippet = "", languageName = "") => {
  ...
  const hasInputMarkers =
    code.includes(BLOCK_MARKERS.inputStart) &&
    code.includes(BLOCK_MARKERS.inputEnd);
  if (!hasInputMarkers) {
    lines = code.split("\n");
    const inputIdx = lines.findIndex((line) => {
      if (family === "cpp" || family === "c")
        return /\bcin\s*>>|scanf\s*\(/.test(line);
      if (family === "java" || family === "kotlin")
        return /scanner\.(next|hasNext)|readLine\s*\(\s*\)/i.test(line);
      if (family === "python") return /\binput\s*\(|sys\.stdin/.test(line);
      if (family === "javascript")
        return /readFileSync\s*\(|input\.split\s*\(/.test(line);
      if (family === "go") return /Scan|ReadString|bufio/.test(line);
      if (family === "dart")
        return /stdin\.readLineSync|readLineSync/.test(line);
      if (family === "php") return /fgets\s*\(\s*STDIN|fgetcsv/.test(line);
      if (family === "r")
        return /readLines\s*\(|file\s*\(\s*['\"]stdin['\"]\s*\)/.test(line);
      ...
      if (family === "ocaml")
        return /Scanf\.scanf|read_line|input_line/.test(line);
    ...
    if (inputIdx !== -1 && solveInvokeIdx !== -1 && solveInvokeIdx > inputIdx) {
      ...
      lines.splice(
        inputIdx,
        0,
        markerLine(family, BLOCK_MARKERS.inputStart, startIndent),
      );
      ...
      lines.splice(
        endIdxShifted,
        0,
        markerLine(family, BLOCK_MARKERS.inputEnd, endIndent),
      );
      code = lines.join("\n");
    }
  }
...
const normalizeMinIndent = (text) => {
  ...
  return lines
    .map((line) => (line.length >= minSpaces ? line.slice(minSpaces) : line))
    .join("\n")
    .trim();
};
...
const injectLogicIntoBoilerplate = ({
  boilerplate,
  todoReplacement,
  languageName,
}) => {
  ...
  if (
    ...
    if (blockLines.length === 0) {
      ...
      const signatureIdx = blockLines.findIndex((line) =>
      ...
      let closingLine = hasClosingBrace
      ...
      if (family === "lua" || family === "octave") {
        const endIdx = blockLines.findIndex(
          (l, i) => i > sigIdx && /^\s*end\s*$/.test(l),
        );
        if (endIdx > sigIdx) {
      ...
      if (family === "ocaml") {
        const semiIdx = blockLines.findIndex(
          (l, i) => i > sigIdx && /;;\s*$/.test(l.trim()),
        );
        if (semiIdx > sigIdx) {
  ...
  if (/TODO|Implement the solution here/i.test(code)) {
    if (family === "python") {
      code = code.replace(/^(\s*)#?\s*.*TODO.*$/im, (_, indent) =>
  ...
  if (family === "python") {
    ...
    let replaced = code.replace(
      /(=\s*do)\s*\n(\s*)[\s\S]*?(\n--\s*HC_IMPLEMENTATION_BLOCK_END)/m,
      (_, eqDo, indent, suffix) => {
        const idt = indent && indent.length > 0 ? indent : "    ";
...
const executeBulk = async ({
  ...
  const safeTimeLimit = Number.isFinite(Number(timeLimit))
    ? Number(timeLimit)
    : 5;
  ...
  const payload = {
    ...
    isAiVerification: isAiVerification || undefined,
  };
  ...
  const response = await withRetry(
    () =>
      model.generateContent(
        buildLogicBlockPrompt({ question, testCases, language }),
      ),
    2,
    500,
  );
 ...
 */
const getFailedTestCaseIndicesToRemove = (
  executionResults,
  threshold = 0.7,
) => {
  ...
  const resultsArray = Array.isArray(firstResult?.results)
    ? firstResult.results
    : [];
  ...
  executionResults.forEach((entry) => {
    ...
    perTc.forEach((r, i) => {
      if (
        i < totalTests &&
        String(r?.status || "").toLowerCase() !== "passed"
      ) {
        failCountByIndex[i]++;
      }
    });
...
`;
  ...
  if (
    !parsed?.completeSolution ||
    typeof parsed.completeSolution !== "string"
  ) {
...
`;
  ...
  return {
    ...
    correctedTestCases:
      Array.isArray(parsed?.correctedTestCases) &&
      parsed.correctedTestCases.length > 0
...
const verifyOneProgrammingQuestion = async ({
  ...
  workingQuestion.supportedLanguages =
    normalizeSupportedLanguages(workingQuestion);

  if (
    !workingQuestion.supportedLanguages.length ||
    !workingQuestion.testCases.length
  ) {
  ...
  const languageStates = (workingQuestion.supportedLanguages || []).map(
    (lang) => ({
      languageId: Number(lang.languageId),
      languageName: lang.languageName,
      ...
      generationFailures.push({
        ...
        summary: {
          ...
          message:
            ...
            `Failed to generate solution for ${state.languageName}`,
        },
        results: [],
        error:
          result.reason?.message ||
  ...
  const runnableExecutions = languageStates
    ...
    .map((ls) => {
      const isPython = detectLanguageFamily(ls.languageName) === "python";
      const codeToSend = isPython
        ? fixPythonIndentationForJudge0(ls.mergedCode)
        : ls.mergedCode;
      return {
  ...
  if (runnableExecutions.length > 0) {
    ...
    totalExecutionUnits +=
      runnableExecutions.length * (workingQuestion.testCases.length || 1);
  }
  if (generationFailures.length > 0)
    executionResults.push(...generationFailures);
  ...
  const originalTestCases = JSON.parse(
    JSON.stringify(workingQuestion.testCases),
  );
  ...
  languagePassSummary.forEach((langResult) => {
    if (isLanguageVerified(langResult)) {
      ...
      const langState = languageStates.find(
        (ls) => ls.languageId === langResult.languageId,
      );
  ...
  if (
    ...
    const phase36SpeculativePromise = (async () => {
      ...
      const results = await Promise.allSettled(
        failingLanguages.map(async (failingLang) => {
          const langState = languageStates.find(
            (ls) => ls.languageId === failingLang.languageId,
          );
          if (!langState) return null;
          const origBp =
            originalBoilerplates.get(failingLang.languageId) ||
            langState.codeSnippet;
          try {
            const completeSolution =
              await generateCompleteSolutionWithBoilerplate({
                genAI,
                modelName,
                question: workingQuestion,
    ...
    const [testCaseEvaluation, speculativeResults] = await Promise.all([
      phase35Promise,
      phase36SpeculativePromise,
    ]);
    ...
    if (testCaseEvaluation && testCaseEvaluation._error) {
      ...
      !testCaseEvaluation?.testCasesCorrect &&
      testCaseEvaluation?.correctedTestCases
    ) {
      ...
      const { languagePassSummary: reLanguagePassSummary } =
        summarizeResults(reExecResults);
  ...
  if (failingLanguages.length > 0) {
    const testCasesToUse = testCasesWereUpdated
      ? workingQuestion.testCases
      : originalTestCases;
    let completeSolutionExecutions;

    if (
      speculativeCompleteSolutions &&
      speculativeCompleteSolutions.length > 0
    ) {
      speculativeCompleteSolutions.forEach(
        ({ langState, completeSolution }) => {
          langState.mergedCode = completeSolution;
        },
      );
      completeSolutionExecutions = speculativeCompleteSolutions.map(
        ({ langState, completeSolution }) => ({
    ...
    if (completeSolutionExecutions && completeSolutionExecutions.length > 0) {
      ...
      totalExecutionUnits +=
        completeSolutionExecutions.length * (testCasesToUse.length || 1);

      // Update results for languages that got complete solutions
      const completeExecMap = new Map(
        completeExecResults.map((res) => [Number(res.languageId), res]),
      );
  ...
  if (REMOVE_FAILED_TEST_CASES && workingQuestion.testCases.length > 1) {
    const allResults = Array.from(executionMap.values());
    const partialPassLangs = languagePassSummary.filter(
      (l) => l.passed < l.total && l.passed > 0,
    );
    ...
    if (partialPassLangs.length >= partialThreshold && allResults.length > 0) {
      ...
      if (
        ...
        indicesToRemove.length < workingQuestion.testCases.length
      ) {
        ...
        languagePassSummary.forEach((langResult) => {
          const entry = allResults.find(
            (r) => Number(r?.languageId) === Number(langResult.languageId),
          );
          const perTc = Array.isArray(entry?.results) ? entry.results : [];
          ...
          indicesToRemove.forEach((idx) => {
            if (
              perTc[idx] &&
              String(perTc[idx]?.status || "").toLowerCase() === "passed"
            ) {
              passedAfterRemoval--;
            }
          });
  ...
  if (failingLanguages.length === 0) {
    const allLanguagesVerified =
      languagePassSummary.length > 0 &&
      languagePassSummary.every((lang) =>
    ...
    return {
      ...
      codeExecutionUnits: totalExecutionUnits,
    };
  }
  ...
  if (hasMajority100Percent) {
    ...
    languagePassSummary.forEach((lang) => {
      verifiedLanguageIds.add(lang.languageId);
      const langState = languageStates.find(
        (ls) => ls.languageId === lang.languageId,
      );
      if (langState) langState.verified = true;
    });
  ...
  if (failingLanguages.length > 0 && ENABLE_PHASE_37) {
    ...
    const runReVerification = async () => {
      ...
      if (revRunnable.length > 0) {
        ...
        totalExecutionUnits +=
          revRunnable.length * (workingQuestion.testCases.length || 1);
      }
      ...
      const revExecMap = new Map(
        revResults.map((res) => [Number(res.languageId), res]),
      );
      ...
      revSummary.forEach((langResult) => {
        if (isLanguageVerified(langResult)) {
          verifiedLanguageIds.add(langResult.languageId);
          const ls = languageStates.find(
            (x) => x.languageId === langResult.languageId,
          );
          if (ls) ls.verified = true;
        } else {
          failingLanguages.push(langResult);
      ...
      if (failingLanguages.length > 0) {
        ...
        const completeExecs = (await Promise.all(completePromises)).filter(
          Boolean,
        );
        if (completeExecs.length > 0) {
          ...
          totalExecutionUnits +=
            completeExecs.length * (testCasesToUse.length || 1);
          compResults.forEach((res) =>
            executionMap.set(Number(res.languageId), res),
          );
  ...
  if (!ENABLE_PHASE_38) {
    ...
    try {
      const failedLangStates = failingLanguages
        .map((fl) =>
      ...
      const bpResult = await generateBoilerplateWithGemini({
        ...
        testCases: workingQuestion.testCases,
        languages: failedLangStates.map((ls) => ({
          languageId: ls.languageId,
          languageName: ls.languageName,
        })),
      });
      ...
      failedLangStates.forEach((ls) => {
        const newCode =
          getBoilerplateForLanguage(bpMap, ls.languageName) || ls.codeSnippet;
      ...
      if (runnable.length > 0) {
        ...
        totalExecutionUnits +=
          runnable.length * (workingQuestion.testCases.length || 1);
      }
      genResults.forEach((r, i) => {
        if (r.status !== "fulfilled") {
          const s = languageStates[i];
      ...
      const { languagePassSummary: finalSummary } =
        summarizeResults(finalExecResults);
      verifiedLanguageIds.clear();
      failingLanguages.length = 0;
      languagePassSummary.length = 0;
...
const verifyProgrammingQuestions = async ({
              ...
              new Promise((_, reject) =>
                setTimeout(
                  () =>
                    reject(
                      new Error(
                        `Verification timeout (${VERIFICATION_QUESTION_TIMEOUT_MS}ms)`,
                      ),
                    ),
                  VERIFICATION_QUESTION_TIMEOUT_MS,
                ),
              ),
            ])
          : await verifyPromise;
      return {
        ...stripInternalFields(result.question),
        codeExecutionUnits: result.codeExecutionUnits || 0,
        verified: result.question?.verified ?? false,
      };
    } catch (error) {
  ...
  const totalCodeExecutionUnits = results.reduce(
    (sum, q) => sum + (q.codeExecutionUnits || 0),
    0,
  );

  // Clean codeExecutionUnits off each question object before returning to caller
  const cleanedResults = results.map((q) => {
    const { codeExecutionUnits: _unused, ...rest } = q;
    return rest;
  });

  return {
    questions: cleanedResults,
    codeExecutionUnits: totalCodeExecutionUnits,
  };
};
